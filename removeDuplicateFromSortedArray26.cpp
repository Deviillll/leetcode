...
int removeDuplicates(vector<int>& nums) {
    ...
    int k = 1; // Start at 1 because the first element is always unique
    for (int i = 1; i < nums.size(); i++) {
        if (nums[i] != nums[i - 1]) {  // If the current number is not the same as the previous
...
int main() {
    vector<int> nums = {1, 1, 2}; // Example input array
